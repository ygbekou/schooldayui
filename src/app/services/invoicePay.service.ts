...
@Injectable()
export class InvoicePayService {
    ...
    public getAll = (invoiceId: number): Observable<InvoicePay[]> => {
        this.actionUrl = Constants.apiServer + '/service/invoicePay/getInvoiceAllInvoicePay/'+invoiceId;

        return this.http.get(this.actionUrl)
            .map((response: Response) => <InvoicePay[]> response.json())
            .catch(this.handleError);
    }
    ...
    public save = (invoiceP: InvoicePay): Observable<InvoicePay> => {
        const toAdd = JSON.stringify(invoiceP);
        const actionUrl = Constants.apiServer + '/service/invoicePay/save';
