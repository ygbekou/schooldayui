...
import {AdminUserSubject} from "./adminUserSubject";
import {FicheRenseignementDashbord} from './ficheRenseignementDashbord';

@Component({
  selector: 'admin-ficheRenseignement',
  templateUrl: '../pages/ficheRenseignement.html',
...
export class FicheRenseignement implements OnInit {
  ...
  @ViewChild(FicheRenseignementDashbord) ficheRenseignementDashbord: FicheRenseignementDashbord;

  @ViewChild(AdminUserSubject) adminUserSubject: AdminUserSubject;
  ...
  COURS_CHOISIS: string = Constants.COURS_CHOISIS;
  ...
  onUserSelected(user: User) {
    ...
    this.adminUserSubject.user = this.user;
    this.changeDetectorRef.detectChanges();
  }
  ...
  onTabChange(evt) {
    ...
    if (evt.index == 0) {
      ...
      this.adminUserSubject.getUserOnlineregistration(this.user);
    }else if (evt.index == 3) {
