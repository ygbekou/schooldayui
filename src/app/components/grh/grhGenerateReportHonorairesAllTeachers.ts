...
@Component({
  selector: 'app-grh-generate-report-honoraires-all-teachers',
  templateUrl: '../../pages/grh/grhGenerateReportHonorairesAllTeachers.html',
  providers: [EmployeeService, Constants]
})

export class GrhGenerateReportHonorairesAllTeachers implements OnInit, OnDestroy {
  ...
  public printEtatHonoraiePeriodAllTeachers() {
    this.reportName=null;
    this.printLabel=Constants.GENERATION_ETAT;

    this.employeeService.printEtatHonoraiePeriodAllTeachers(this.paySlip)
    .subscribe((data: string) => {
      this.reportName = data;
      this.printLabel=Constants.GENERER_ETAT;
    },
    error => console.log(error),
    () => console.log('Get print EtatHonoraiePeriodAllTeachers'));
  }
