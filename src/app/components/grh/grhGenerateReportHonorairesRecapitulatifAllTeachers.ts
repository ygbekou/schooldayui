...
@Component({
  selector: 'app-grh-generate-report-honoraires-recapitulatif-all-teachers',
  templateUrl: '../../pages/grh/grhGenerateReportHonorairesRecapitulatifAllTeachers.html',
  providers: [EmployeeService, Constants]
})

export class GrhGenerateReportHonorairesRecapitulatifAllTeachers implements OnInit, OnDestroy {
  ...
  public printEtatHonoraieRecapitulatifPeriodAllTeachers() {
    this.reportName=null;
    this.printLabel=Constants.GENERATION_ETAT;

    this.employeeService.printEtatHonoraieRecapitulatifPeriodAllTeachers(this.paySlip)
    .subscribe((data: string) => {
      this.reportName = data;
      this.printLabel=Constants.GENERER_ETAT;
    },
    error => console.log(error),
    () => console.log('Get print EtatHonoraireRecapitulatifPeriodAllTeachers'));
  }
