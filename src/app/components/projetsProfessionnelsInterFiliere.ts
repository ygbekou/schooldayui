...
@Component({
    moduleId: 'module.id',
    selector: 'app-projets-profesionnels-inter-filiere',
    templateUrl:'../pages/projetsProfessionnelsInterFiliere.html',
    providers: [StudentProjectService]
})

export class ProjetsProfesionnelsInterFiliere implements OnInit, AfterViewInit{
    ...
    ngOnInit() {
        ...
        if(this.user.id > 0){
            this.getAllInterFiliereThemeWithProjects();
        }else{
    ...
    public getAllInterFiliereThemeWithProjects() {
        this.studentProjectService.getAllInterFiliereThemeWithProjects().subscribe((data: StudentProjectCategory[]) => {
                this.themes = data;
            },
