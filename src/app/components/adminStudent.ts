...
import { ManageStudentKioskCard } from './manageStudentKioskCard';
import { ManageStudentParent } from './manageStudentParent';
@Component({
  selector: 'app-admin-student',
  templateUrl: '../pages/adminStudent.html',
  providers: [{ provide: LOCALE_ID, useValue: Constants.LOCALE} ]
})
export class AdminStudent implements OnInit{
  ...
  @ViewChild(ManageStudentKioskCard) manageStudentKioskCard: ManageStudentKioskCard;
  @ViewChild(ManageStudentParent) manageStudentParent: ManageStudentParent;

  public user: User;
  public student: Student;
  public activeTab = 0;
  public activeSubProfileTab = 0;
  ...
  onUserSelected(user: User) {
    ...
    this.manageStudentKioskCard.setStudent(this.user);
  ...
  onSubProfileTabChange(evt) {
    this.activeSubProfileTab = evt.index;
    if (evt.index === 0) {

    } else if (evt.index === 1) {

    } else if (evt.index === 2) {
      this.manageStudent.getLastStudentBulletinAndDegree();
