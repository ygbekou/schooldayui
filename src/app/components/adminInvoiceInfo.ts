...
@Component({
    selector: 'app-admin-invoice-info',
    templateUrl: '../pages/adminInvoiceInfo.html',
    providers: [UserService, Constants]
})

export class AdminInvoiceInfo implements OnInit {
    success = '';
    roles: any[] = [];
    ...
    @Output() onCancelInvoiceUpdateSelected = new EventEmitter<InvoiceView>();
    ...
    public cancelUpdateInvoice()
    {
        this.onCancelInvoiceUpdateSelected.emit(this.nInvoiceView);
    }
    ...
    onVendeurSelected(event) {
       ...
       // this.nInvoice.vendeur = event.data;
    }
    ...
    public update() {
        ...
        //Si les conditions de ventes ne sont pas saisies
            ...
            try {
                ...
                this.nInvoiceView.paymentDate = new Date(this.nInvoiceView.paymentDate);
                this.nInvoiceView.vendeurId = this.selectedUser.id;
                this.invoiceService.update(this.nInvoiceView)
                    .subscribe(result => {
    ...
    searchVendurUser(){
        ...
        if (this.searchTextUser != null) {
            this.userService.search(this.searchTextUser).subscribe((data: User[]) => {
                    this.vendeurs = data;
                    if (this.vendeurs == null || this.vendeurs.length <= 0) {
                        this.error = Constants.NO_USER_FOUND;
                    }
                },
                error => console.log(error),
    ...
    public getInvoiceViewById(invoiceView: InvoiceView) {
        ...
        this.invoiceService.getInvoiceViewById(invoiceView).subscribe((data: InvoiceView) => {
            ...
            error => console.log(error),
            () => console.log('Find invoice view by id ' + invoiceView.id));
    }

    public sendToClient() {
        this.error = null;
        this.invoiceService.sendToClient(this.nInvoiceView).subscribe((data: InvoiceView) => {
                this.nInvoiceView = data;

                this.msgs.push({severity: 'success', summary: 'Succès', detail: 'Etat changé en envoyé au client !'});
            },
            error => console.log(error),
            () => console.log('Sent invoice id = ' + this.nInvoiceView.invoiceId + ' complete'));
    }

    public clientAprove() {
        this.error = null;
        this.invoiceService.clientApproveInvoiceByView(this.nInvoiceView).subscribe((data: InvoiceView) => {
                this.nInvoiceView = data;
                console.log(data);
            ...
            error => console.log(error),
            () => console.log('Approve invoice id = ' + this.nInvoiceView.invoiceId + ' complete'));
    }

    public clientReject() {
        this.error = null;
        this.invoiceService.clientReject(this.nInvoiceView).subscribe((data: InvoiceView) => {
                this.nInvoiceView = data;

                this.msgs.push({severity: 'success', summary: 'Succès', detail: 'Etat changé en rejecté par le client !'});
            },
            error => console.log(error),
            () => console.log('Reject invoice id = ' + this.nInvoiceView.invoiceId + ' complete'));
    }
}
